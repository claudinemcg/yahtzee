...
class FullHouse extends Rule {
  evalRoll = dice => {
    const freqDice = this.freq(dice);
    return (freqDice.includes(2) && freqDice.includes(3)) ? this.score : 0;
  }
}
...
const fullHouse = new FullHouse({score: 25, description: '25 points if 3 & 2 are same'});

// small/large straights score as 30/40
const smallStraight = new SmallStraight({score: 30, description: '30 points if 4 values in a row'});
const largeStraight = new LargeStraight({ score: 40, description: '40 points if 5 values in a row' });

// yahtzee scores as 50
const yahtzee = new Yahtzee({ score: 50, description: '50 points if all values are same'});
